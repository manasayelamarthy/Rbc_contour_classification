"""
Re-scan every box in the (now-restored) training/val labels for WBC coverage,
using an 80% threshold instead of the earlier 35% -- per calibration against
confirmed real-WBC vs confirmed false-positive-RBC boxes, 80% cuts false
positives sharply (only 1/7 confirmed-false-positive samples reached 0.80)
while still catching the more obviously WBC-dominated boxes.
"""
...
OUT = Path(r"F:\Livo\Data - 2026\Rbc\wbc_flagged_80pct")
...
for split in ("train", "val"):
    ...
    for label_path in sorted(lbl_dir.glob("*.txt")):
        ...
        if image_flags:
            ...
            # save an overlay: green = all boxes, red = the flagged one(s)
            vis = img.copy()
            for line in lines:
                cls, cx, cy, w, h = [float(v) for v in line.split()]
...
print(f"{len(flagged)} images flagged, {n_flagged_boxes} boxes flagged at >=80% WBC coverage")
